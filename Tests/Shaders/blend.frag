...
vec2 SMAAArea(sampler2D areaTex, vec2 dist, float e1, float e2, float offset) {
    ...
    vec2 texcoord = fma(vec2(SMAA_AREATEX_MAX_DISTANCE, SMAA_AREATEX_MAX_DISTANCE), floor((4.0 * vec2(e1, e2)) + 0.5), dist);
...
void main() {
    ...
    if (e.g > 0.0) { // Edge at north
        ...
        weights.rg = SMAACalculateDiagWeights(edge, area, inUV, e, subsampleIndices);
        ...
        if (weights.r == -weights.g) { // weights.r + weights.g == 0.0
            ...
            vec3 coords;
            coords.x = SMAASearchXLeft(edge, search, vOffset0.xy, vOffset2.x);
            coords.y = vOffset1.y; // vOffset[1].y = vTexCoord0.y - 0.25 * SMAA_RT_METRICS.y (@CROSSING_OFFSET)
            d.x = coords.x;

            // Now fetch the left crossing edges, two at a time using bilinear
            // filtering. Sampling at -0.25 (see @CROSSING_OFFSET) enables to
            // discern what value each edge has:
            float e1 = texture(edge, coords.xy).r; // LinearSampler

            // Find the distance to the right:
            coords.z = SMAASearchXRight(edge, search, vOffset0.zw, vOffset2.y);
            d.y = coords.z;

            // We want the distances to be in pixel units (doing this here allow to
            // better interleave arithmetic and memory accesses):
            d = abs(floor(fma(SMAA_RT_METRICS.zz, d, -vPixCoord.xx) + 0.5));
            ...
            float e2 = SMAASampleLevelZeroOffset(edge, coords.zy, vec2(1, 0)).r;

            // Ok, we know how this pattern looks like, now it is time for getting
            // the actual area:
            weights.rg = SMAAArea(area, sqrt_d, e1, e2, subsampleIndices.y);

            // Fix corners:
            coords.y = inUV.y;
            SMAADetectHorizontalCornerPattern(edge, weights.rg, coords.xyzy, d);
    ...
    if (e.r > 0.0) { // Edge at west
        ...
        vec3 coords;
        coords.y = SMAASearchYUp(edge, search, vOffset1.xy, vOffset2.z);
        coords.x = vOffset0.x; // vOffset[1].x = vTexCoord0.x - 0.25 * SMAA_RT_METRICS.x;
        d.x = coords.y;

        // Fetch the top crossing edges:
        float e1 = texture(edge, coords.xy).g; // LinearSampler

        // Find the distance to the bottom:
        coords.z = SMAASearchYDown(edge, search, vOffset1.zw, vOffset2.w);
        d.y = coords.z;

        // We want the distances to be in pixel units:
        d = abs(floor(fma(SMAA_RT_METRICS.ww, d, -vPixCoord.yy) + 0.5));
        ...
        float e2 = SMAASampleLevelZeroOffset(edge, coords.xz, vec2(0, 1)).g;

        // Get the area for this direction:
        weights.ba = SMAAArea(area, sqrt_d, e1, e2, subsampleIndices.x);

        // Fix corners:
        coords.x = inUV.x;
        SMAADetectVerticalCornerPattern(edge, weights.ba, coords.xyxz, d);
    }
